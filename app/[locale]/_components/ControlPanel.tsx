...
export default function ControlPanel({
  ...
  const tTempLabels = useTranslations("TempLabels");
  const tIndex = useTranslations("Index");

  const getTemperatureLabel = (value: number) => {
    if (value <= 0.5) return tTempLabels("cool");
    if (value <= 1.1) return tTempLabels("classic");
    return tTempLabels("warsaw");
  };
  ...
  return (
    <div className="flex flex-col gap-8 h-full w-full p-2 mt-4 font-mono">
      ...
      >
        {isGenerating ? tIndex("generating") : tIndex("generate")}
      </button>
    </div>
  );
}
